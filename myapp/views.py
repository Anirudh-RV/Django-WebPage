...
def ajaxfunc(request):
    ...
    print(msg)
    return render(request, 'visual.html', {'posts': msg})
...
def index(request):
    ...
    with open('final_11.csv') as csvfile:
        readCSV = csv.reader(csvfile, delimiter=',')
        for row in readCSV:
            for i in row:
                global_csv_file_string = global_csv_file_string + i + ","

            global_csv_file_string = global_csv_file_string + "\r\n"


    print(global_csv_file_string)
...
def visualisation(request):
    ...
    # this gloval_csv_file_string has to be paras du csv file du decoded value

    """
data_set = paras_csv_file.read().decode('utf-8')
